...
def main(args=None):
    ...
    if len(comb_virus_data_df) > 0:
        ...
        comb_virus_data_df.loc[:,"taxon_for_threshold"] = comb_virus_data_df.apply(
            lambda row: fetch_taxon(thresholds_dict, row["lineage"]),
            axis=1
            )

        # map taxon information onto df
        thresholds_dict[None] = 1000000
        comb_virus_data_df["taxon_min_length"] = comb_virus_data_df.taxon_for_threshold.map(lambda x: thresholds_dict[x])
